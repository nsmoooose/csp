...
bool CFrustum::PointInFrustum(CVector vPos)
{
	for (int iCurPlane = 0; iCurPlane<6; iCurPlane++)
	{
		if (m_fFrustum[iCurPlane][0] * vPos.x + m_fFrustum[iCurPlane][1] * 
			vPos.y + m_fFrustum[iCurPlane][2] * vPos.z + m_fFrustum[iCurPlane][3] <= 0)
		{
			return false;
		}
	}
...
bool CFrustum::QuadInFrustum(CVector vPos, float fSize, float fHeight)
{
	for (int iCurPlane = 0; iCurPlane<6; iCurPlane++)
	{
		//if (iCurPlane==2 || iCurPlane==3) fLength = fHeight;
		//	else fLength = fSize;
		if (m_fFrustum[iCurPlane][0] * (vPos.x - fSize) + m_fFrustum[iCurPlane][1] *
			(vPos.y - fSize) + m_fFrustum[iCurPlane][2] * (vPos.z - fHeight) + 
