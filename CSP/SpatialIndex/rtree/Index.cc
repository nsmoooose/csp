...
NodePtr Index::chooseSubtree(const Region& mbr, unsigned long level, std::stack<long>& pathBuffer)
{
	if (m_level == level) return NodePtr(this, &(m_pTree->m_indexPool));
	...
	NodePtr ret = n->chooseSubtree(mbr, level, pathBuffer);
	assert(n.unique());
...
void Index::split(unsigned long dataLength, byte* pData, Region& mbr, long id, NodePtr& pLeft, NodePtr& pRight)
{
...
void Index::adjustTree(Node* n, std::stack<long>& pathBuffer)
{
	...
	bool b = m_nodeMBR.containsRegion(n->m_nodeMBR);
	bool recalc = (! b) ? true : m_nodeMBR.touchesRegion(*(m_ptrMBR[child]));

	*(m_ptrMBR[child]) = n->m_nodeMBR;

	if (recalc && m_pTree->m_bTightMBRs)
	{
...
void Index::adjustTree(Node* n1, Node* n2, std::stack<long>& pathBuffer, byte* overflowTable)
{
	...
	//   1. the NEW child MBR is not contained.
	//   2. the OLD child MBR is touching.
	bool b = m_nodeMBR.containsRegion(n1->m_nodeMBR);
	bool recalc = (! b) ? true : m_nodeMBR.touchesRegion(*(m_ptrMBR[child]));
	...
	*(m_ptrMBR[child]) = n1->m_nodeMBR;
	...
	if ((! bAdjusted) && recalc && (! pathBuffer.empty()))
	{
		long cParent = pathBuffer.top(); pathBuffer.pop();
		NodePtr ptrN = m_pTree->readNode(cParent);
		Index* p = static_cast<Index*>(ptrN.get());
