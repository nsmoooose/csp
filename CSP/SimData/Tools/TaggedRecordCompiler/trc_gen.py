...
class Header(Toplevel):
	...
	def writeSource(self, output):
		...
		output('#include <SimData/TaggedRecordRegistry.h>')
		output.newline()
...
# elements (contents of messages and groups)
...
class Element:
	...
	def getHasFlag(self):
		assert(not self.hasflag is None)
		return self.hasflag
...
class Compound(Element):
	...
	def writeChildHasFields(self, output):
		...
		for child in self.children:
			...
			if (idx == 0):
				...
				output('simdata::uint32 %s;' % has_var)
	...
	def writeSerialize(self, output):
		output('void serialize(simdata::TagWriter &writer) const;')
		output('void serialize(simdata::TagReader &reader);')
		output.newline()

	def writeSerializeSource(self, output, scope=''):
		typename = scope + self.type.name
		output('void %s::serialize(simdata::TagWriter &writer) const {' % typename)
		output.indent()
		...
		output.dedent()
		output('}\n')
		output('void %s::serialize(simdata::TagReader &reader) {' % typename)
		output.indent()
	...
	def writeDump(self, output, name=None):
		...
		output('void dump(simdata::DumpWriter &out, const char *name=0) const;')
		output.newline()

	def writeDumpSource(self, output, scope=''):
		typename = scope + self.type.name
		output('void %s::dump(std::ostream &, int) const {}' % typename)
		output('void %s::dump(simdata::DumpWriter &out, const char *name) const {' % typename)
		output.indent()
		output('if (name) {')
		output.indent()
	...
	def writeRef(self, output):
		output('typedef simdata::Ref<%s> Ref;' % self.type.name)
...
class Field(Element):
	...
	def __str__(self):
		...
		if self.array: array = '[]'
		pre = ''
		if self.deprecated: pre = 'DEP '
		elif self.required: pre = 'REQ '
		default = ''
...
class Group(Compound):
	"""
	A class representing a group definition.  Groups are simdata::Group subclasses
	...
	def write(self, output):
		output('class %s : public simdata::TaggedGroup {' % (self.type.name))
		output.indent()
	...
	def __str__(self):
		dep = ''
		if self.deprecated: dep = 'DEP '
		return '%sgroup %s {\n%s\n}' % (dep, self.name, '\n'.join(map(str, self.body)))

	def getType(self):
		return 'simdata::Ref<%s>' % self.type.name
...
class Bitset(Compound):
	...
	def getType(self):
		if len(self.children) <= 8:
			return 'simdata::uint8'
		if len(self.children) <= 16:
			return 'simdata::uint16'
		assert(len(self.children) <= 32)
		return 'simdata::uint32'
...
class Message(Toplevel, Compound):
	...
	def writeBoilerPlate(self, output):
		...
		output('static const Id _Id = SIMDATA_ULL(%d);' % id64)
	...
	def writeHeader(self, output):
		...
		if self.base:
			...
			output('class %s : public simdata::TaggedRecord {' % (self.name))
		output.indent()
		self.writeChildren(output)
		output.dedent()
		output('};')
	...
	def writeSource(self, output):
		...
		output('namespace { simdata::TaggedRecordFactory<%s> __%s_factory; }' % (self.name, self.name))
		output.newline()
