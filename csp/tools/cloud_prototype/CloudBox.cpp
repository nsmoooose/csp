...
void CloudBox::UpdateModel() {
	...
	osg::Vec3 cloudCenter(m_Width/2, m_Height/2, m_Depth/2);
	...
	for(int i = 0;i<100;++i) {
		...
		osg::Vec3 position((rand() % (int)m_Width), 
			(rand() % (int)m_Height), (rand() % (int)m_Depth));
		...
		float colorLevel = position.y() / m_Height;
		...
		sprite->UpdateModel(position, dissipation, selectedColorLevel.color);
