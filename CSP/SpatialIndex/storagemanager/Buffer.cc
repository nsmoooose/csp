	...
	m_hits(0)
{
	...
	{
		if (var.varType != VT_ULONG) throw IllegalArgumentException("Property Capacity must be VT_ULONG");
		m_capacity = var.val.ulVal;
	}

	var = ps.getProperty("WriteThrough");
	if (var.varType != VT_EMPTY)
	{
		if (var.varType != VT_BOOL) throw IllegalArgumentException("Property WriteThrough must be VT_BOOL");
		m_bWriteThrough = var.val.blVal;
	}
}
