...
class TypeArchiveTest(ArchiveTest):
	...
	def testReal(self):
		...
		self.archive._Real(x0)
		self.assertEqual(x0.mean(), y0.mean())
		self.assertEqual(x0.sigma(), y0.sigma())
		self.setRead()
		self.assertEqual(self.size, 8)
		x1 = self.archive._Real()
		self.assertEqual(x0.mean(), x1.mean())
		self.assertEqual(x0.sigma(), x1.sigma())
		v = x1.value()
		# 5 sigma test ;-)
