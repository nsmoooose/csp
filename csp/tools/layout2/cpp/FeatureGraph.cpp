...
csp::layout::FeatureGraph::FeatureGraph(): m_Scene(new osg::Group), m_RootGroup(new LayoutGroup), m_ActiveGroup(0) {
	m_RootGroup->setName("default_group");
	m_Scene->addChild(m_RootGroup->getGraph());
	activateRoot();
}

csp::layout::FeatureGraph::~FeatureGraph() {
	deleteBelowRoot();
	delete m_RootGroup;
}

void csp::layout::FeatureGraph::setRoot(LayoutGroup *group) {
	deleteBelowRoot();
	m_ActiveGroup = 0;
	delete m_RootGroup;
	if (!group) {
...
void csp::layout::FeatureGraph::signalActivate() {
...
void csp::layout::FeatureGraph::clearAllSelections() {
	ClearSelectionVisitor v;
	m_RootGroup->accept(v);
}

void csp::layout::FeatureGraph::activateRoot() {
...
void csp::layout::FeatureGraph::activateParent() {
...
void csp::layout::FeatureGraph::activateChild(LayoutGroup *group) {
...
void csp::layout::FeatureGraph::deepSelect(LayoutNodePath const &node_path) {
	activateRoot();
...
osg::Matrix csp::layout::FeatureGraph::getActiveGroupToWorldMatrix() const {
	osg::Matrix m;
...
void csp::layout::FeatureGraph::select(LayoutNode *node) {
...
void csp::layout::FeatureGraph::deselect(LayoutNode *node) {
	node->setSelected(false);
	for (GraphCallbackList::iterator iter = m_GraphCallbacks.begin(); iter != m_GraphCallbacks.end(); ++iter) {
...
void csp::layout::FeatureGraph::deselectAll() {
...
void csp::layout::FeatureGraph::signalRefresh() {
	for (GraphCallbackList::iterator iter = m_GraphCallbacks.begin(); iter != m_GraphCallbacks.end(); ++iter) {
		(*iter)->onRefresh();
	}
}

void csp::layout::FeatureGraph::deleteBelowRoot() {
...
void csp::layout::FeatureGraph::undo() {
	m_CommandHistory.undo();
	std::cout << "undo...\n";
	signalRefresh();
}

void csp::layout::FeatureGraph::redo() {
	m_CommandHistory.redo();
	std::cout << "redo...\n";
	signalRefresh();
}

void csp::layout::FeatureGraph::clearHistory() {
	m_CommandHistory.clear();
}

void csp::layout::FeatureGraph::runCommand(Command *command) {
	m_CommandHistory.runCommand(command);
	std::cout << "run command...\n";
	signalRefresh();
}

void csp::layout::FeatureGraph::addCommand(Command *command) {
	m_CommandHistory.addCommand(command);
}

csp::layout::CallbackId csp::layout::FeatureGraph::addCallback(GraphCallback *callback) {
	assert(callback);
	m_GraphCallbacks.push_back(callback);
	return reinterpret_cast<CallbackId>(callback);
}

void csp::layout::FeatureGraph::removeCallback(CallbackId id) {
	for (GraphCallbackList::iterator iter = m_GraphCallbacks.begin(); iter != m_GraphCallbacks.end(); ++iter) {
...
void csp::layout::FeatureGraph::accept(LayoutNodeVisitor &visitor) {
	m_RootGroup->accept(visitor);
}

void csp::layout::GraphCallback::onSelect(LayoutNodePath*, LayoutNode*) { }
void csp::layout::GraphCallback::onDeselect(LayoutNodePath*, LayoutNode*) { }
void csp::layout::GraphCallback::onActivate(LayoutNodePath*, osg::Matrix const &) { }
void csp::layout::GraphCallback::onSelectionClear() { }
void csp::layout::GraphCallback::onRefresh() { }
