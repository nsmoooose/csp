...
namespace simdata { class Vector3; }
...
protected:
	...
	public:
		 ...
		 */
		ObjectWrapper(ObjectId id, simdata::Path const &path, GridCoordinate x, GridCoordinate y): QuadTreeChild(id, x, y), m_Path(path) { }
		 ...
		 */
		inline const simdata::Path path() const { return m_Path; }
	...
	public:
		...
		UnitWrapper(ObjectId id, simdata::Path const &path, PeerId owner);
