...
CMipMapQuad::CMipMapQuad(int iXPos, int iYPos, int iLength, int iMaxSteps, CTerrainData *pTerrainData)
{
	...
	m_fSize			= iLength/2;
	m_afD			= new float[(int)(log(m_iMaxSteps)/log(2))];	
	m_bVisible		= false;
	m_iCurSteps		= 1;
	CalcMinMaxHeight();
}
...
int CMipMapQuad::RenderQuad(int iStepTop, int iStepBottom, int iStepLeft, int iStepRight, bool bLines)
{
	...
	if (iStepTop>m_iCurSteps)	// has the upper quad a lower resolution than this one?
	{
		// yes, obviously!
		iTriangleCount += RenderInterpolation((float)m_pTerrainData->m_iWidth-1-m_iCurSteps, (float)m_iCurSteps, m_pTerrainData->m_afHeightmap+(m_iCurSteps+1)*m_pTerrainData->m_iWidth-1-m_iCurSteps, 
			...
			(float)m_iCurSteps, (float)m_iCurSteps);
		iYStart = m_iCurSteps;		// the first "line" of triangles mustn't be drawed again (where the crack-resolving took place)
		...
		if (iStepLeft<=m_iCurSteps) // were cracks resolved on the left side?
		{
			iTriangleCount++; 
			DrawTriangle(0, 0, m_iCurSteps, m_iCurSteps, 0, m_iCurSteps); // yes? then let's fill the resulting gap
		}
	...
	if (iStepBottom>m_iCurSteps)
	{
		iTriangleCount += RenderInterpolation((float)m_iCurSteps, (float)m_pTerrainData->m_iHeight-m_iCurSteps-1, m_pTerrainData->m_afHeightmap+(m_pTerrainData->m_iHeight-1-m_iCurSteps)*m_pTerrainData->m_iWidth+m_iCurSteps, 
			...
			(float)m_pTerrainData->m_iWidth-1-m_iCurSteps, (float)m_pTerrainData->m_iHeight-1-m_iCurSteps);
		iYEnd = m_iCurSteps;
	...
	{ 
		iTriangleCount += RenderInterpolation((float)m_iCurSteps, (float)m_iCurSteps, m_pTerrainData->m_afHeightmap+m_iCurSteps*m_pTerrainData->m_iWidth+m_iCurSteps, 
			...
			(float)m_iCurSteps, (float)m_pTerrainData->m_iHeight-1-m_iCurSteps);
		iXStart = m_iCurSteps;
	...
	{  
		iTriangleCount += RenderInterpolation((float)m_pTerrainData->m_iWidth-1-m_iCurSteps, (float)m_pTerrainData->m_iWidth-1-m_iCurSteps, m_pTerrainData->m_afHeightmap+(m_pTerrainData->m_iHeight-m_iCurSteps)*m_pTerrainData->m_iWidth-m_iCurSteps-1, 
			...
			(float)m_pTerrainData->m_iWidth-1-m_iCurSteps, (float)m_iCurSteps);
		iXEnd = 1;
		if (iStepTop<=m_iCurSteps)
		{
			iTriangleCount++;
...
int CMipMapQuad::CalcEpsilon(float fTao, float fNearClip, float fTopCoor, int iVRes)
{
	int		iDIndex	= 0;
...
inline void CMipMapQuad::DrawTriangle(int iX1, int iY1, int iX2, int iY2, int iX3, int iY3)
{
