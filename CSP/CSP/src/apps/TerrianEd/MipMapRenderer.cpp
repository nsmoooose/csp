...
void CMipMapRenderer::Render(float fPosX, float fPosY, float fPosZ, float fRotX, float fRotY, bool bLines)
{
	...
	fontDrawString(0, 0, "NoTriangles: %d, fRenderDistance: %.1f, %d, %d, m_fRenderDistance: %.1f", iNoTriangles, fRenderDist, iCount1, iCount2, m_fRenderDistance);
}
...
/*			m_MipmapQuads[y*m_iQuadWidth+x]->CalcEpsilon(m_fDetailLevel, 0.1f, 0.08f, 480, 1); */
			m_MipmapQuads[y*m_iQuadWidth+x]->CalcEpsilon(m_fDetailLevel, EngineStatus.m_fEpsilonNearClip,
														 EngineStatus.m_fEpsilonTopCoor, 480, EngineStatus.m_fEpsilonDeltaH);
