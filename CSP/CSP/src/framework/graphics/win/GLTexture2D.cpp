...
int const GLTexture2D::DIB	= 1;
int const GLTexture2D::TGA	= 2;
short GLTexture2D::mipmaplevels = 0;

GLTexture2D::GLTexture2D(StandardGraphics * pDisplay) : Texture2D(pDisplay)
{
	texID		= 0;
	width		= 0;
	height		= 0;
...
void GLTexture2D::Create( const char *filename, int filetype )
{
...
bool GLTexture2D::genTexObject( const char *filename, int filetype )
{
#ifdef	LINUX
	glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL, 0 );
	glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, mipmaplevels );
...
unsigned int GLTexture2D::getTexID()
{
	return( texID );
}

void GLTexture2D::setIntFormat( unsigned long format )
{
	intFormat = format;
}

bool GLTexture2D::setMinFilter( unsigned long filter )
{
	if( minFilter != filter )
	{
		minFilter = filter;
...
bool GLTexture2D::setMagFilter( unsigned long filter )
{
	if( magFilter != filter )
	{
		magFilter = filter;
...
bool GLTexture2D::setWrapS( unsigned long wrap )
{
	if( wrapS != wrap )
	{
		wrapS = wrap;
...
bool GLTexture2D::setWrapT( unsigned long wrap )
{
	if( wrapT != wrap )
	{
		wrapT = wrap;
...
bool GLTexture2D::setWrapR( unsigned long wrap )
{
	if( wrapR != wrap )
	{
		wrapR = wrap;
...
void GLTexture2D::LoadDIBitmap( const char *filename, DIBINFO *info )
{
...
void GLTexture2D::WriteDIBitmap( DIBINFO *image )
{

}

void GLTexture2D::LoadTGA( const char *filename, TGAINFO *info )
{
	ifstream inTGA;
...
void GLTexture2D::WriteTGA( TGAINFO *info )
{
