#ifndef _LinuxNetwork__H_
#define _LinuxNetwork__H_
...
//! System-dependent (linux) Network implementation.
/*! \class LinuxNetwork
 *  This class should be the lowest netcode level. 
 *  It is system dependent to linux. On the top of this class (StandardNetwork) the netcode is independent from system. 
 ...
 *	The server uses LinuxNetwork::Create to establish a socket connected to local address and defined port.
 *	Next it uses LinuxNetwork::Listen to inform the system that it await connect requests from clients. 
 *	Now this network is only for receiving connection requests and build up new connections.  
 *	With LinuxNetwork::Accept the server is checking for this requests It must do this periodicaly.
 *	If a client want to connect, the server receives that on LinuxNetwork::Accept an build up an new connection.
 *	This new connection will used to communicate with this special client.
 *	The client uses LinuxNetwork::Create to establish a socket connected to local address. The port should set to 0.
 *	If port is set to 0 the system is selecting an free port. The next step is to connect with LinuxNetwork::Connect to the server.
 *	After establishing these connection the client can communicate with defined server.<br>
 *	- Using UDP/IP:<br>That's more simple. At first use LinuxNetwork::Create to establish a socket connected to local address.
 *	Servers must define the port (clients should use port = 0). 
 *	After that client and server could use LinuxNetwork::Connect to define a default destination. So LinuxNetwork::Send and LinuxNetwork::Receive are usable.
 *	Without doing that, only LinuxNetwork::SendTo and LinuxNetwork::ReceiveFrom to receive from or send to a special destination.
 *	Without specifying the default destination ::Receive will receive from all destinations (I think so :)). That's usefull on servers.
 *
 *  \author Paracelsius74
 *  \version 0.1
 *  \date    20. February 2001
 *  \todo Check max_packet_size on UDP/IP.
 *	\todo Add on some methods time-depended execution. (LinuxNetwork::Send, LinuxNetwork::SendTo, LinuxNetwork::Connect).
 */
class LinuxNetwork: public StandardNetwork{
public:	
   ...
   */
   LinuxNetwork(SOCKET _initializedSocket = INVALID_SOCKET);
   //! A destructor.
   /*!
      It destroys the socket. It calls LinuxNetwork::Close() for that.
    */
	virtual ~LinuxNetwork(void);
	//@}
	...
	*/
	short Create(unsigned short _socketType,long &_ip, short &_port);
	...
	*/
	...
	/*! This socket will used after execution only for initial handshaking between client and server. 
		So this method is ever used together with LinuxNetwork::Accept.
	...
	*/
	...
	/*! Before using this method LinuxNetwork_Listen must be executed. The method is _mSecs microseconds waiting for an incoming connection request.
	...
	*/
	...
	//! Build up a connection to foreign address (TCP/IP) or define a default destination for StandardNetwork::Send and LinuxNetwork::Receive (UDP/IP).
	/*! On UDP/IP: Any datagrams received from an address other than the destination address specified will be discarded. 
	If the ip-address is zero, the socket will be disconnected. Then no default destination is set and only LinuxNetwork::SendTo and LinuxNetwork::ReceiveFrom are usable.<br>
	...
	*/
	...
	//! Receives packets. If default destination is set (LinuxNetwork::Connect) it receives only packets from this address. 
	...
	*/
	...
	For UDP/IP, only the address of a peer specified in a previous LinuxNetwork::Connect call will be returned.
	Any address specified by a previous LinuxNetwork::SendTo call will not be returned by this method.
	...
	*/
	//static struct hostent *GetHostByName(const char *_name);
	...
	*/
	//static struct hostent *GetHostByAddress(struct sockaddr_in &_addr);
	//@}
protected:
	//! If on a socket method occured an error this method will check the kind of error and writes a small message into the file.
	/*!
	...
	*/
	static short CheckError(FILE *_fp,char *_string = NULL);
