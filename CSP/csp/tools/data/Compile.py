...
try:
	...
	print "Unable to import the csplib module."
	sys.exit(1)

try:
	from csp import cspsim
except:
	print "Unable to import the cspsim module."
	sys.exit(1)
...
class Compiler:
	...
	def printUsage(self, out=sys.stdout):
		print >>out, "CSP XML data compiler"
		if self.standalone:
		...
		for arg in args:
			if arg.startswith('-'):
				if arg.startswith('--warn='):
					...
					if level > 0:
						csplib.log().setPriority(csplib.LogStream.cWarning)
					...
					setDebugLevel(level)
					if level > 1:
						csplib.log().setPriority(csplib.LogStream.cDebug)
					elif level > 0:
						csplib.log().setPriority(csplib.LogStream.cInfo)
				elif arg == '--rebuild':
					self.rebuild = 1
				elif arg == '--force':
					self.force = 1
