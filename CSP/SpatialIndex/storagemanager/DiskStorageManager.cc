...
SpatialIndex::IStorageManager* SpatialIndex::StorageManager::returnDiskStorageManager(PropertySet& ps)
	...
	throw (std::exception, SpatialIndex::Exception)
{
	...
	var.varType = VT_PCHAR;
	var.val.pcVal = const_cast<char*>(baseName.c_str());
	ps.setProperty("FileName", var);
		// .idx and .dat extensions will be added.

	var.varType = VT_ULONG;
	var.val.ulVal = pageSize;
...
DiskStorageManager::DiskStorageManager(PropertySet& ps) : m_pageSize(0), m_nextPage(-1), m_buffer(0)
{
	Variant var;
	...
	{
		if (var.varType != VT_BOOL) throw IllegalArgumentException("Property Overwrite must be VT_BOOL");
		bOverwrite = var.val.blVal;
	}
	...
	if (var.varType != VT_EMPTY)
	{
		if (var.varType != VT_PCHAR) throw IllegalArgumentException("Property FileName must be VT_PCHAR");

		int cLen = strlen(var.val.pcVal);

		char* pIndexFile = new char[cLen + 10];
		char* pDataFile = new char[cLen + 10];

		sprintf(pIndexFile, "%s.idx", var.val.pcVal);
		sprintf(pDataFile, "%s.dat", var.val.pcVal);
		...
		if (bOverwrite == false)
		{
			...
			if ((e1 != 0 || e2 != 0) && bFileExists)
			{
				delete[] pIndexFile;
				delete[] pDataFile;
				throw IllegalArgumentException("Index file cannot be read/writen.");
			}
		}
		...
		if (m_indexFile < 0)
		{
			delete[] pIndexFile;
			delete[] pDataFile;
			throw IllegalArgumentException("Index file cannot be opened.");
		}
		...
		if (m_dataFile < 0)
		{
			delete[] pIndexFile;
			delete[] pDataFile;
			throw IllegalArgumentException("Data file cannot be opened.");
		}
	...
	{
		throw IllegalArgumentException("Property FileName was not specified.");
	}
	...
	if (bOverwrite == true)
	{
		...
		if (var.varType != VT_EMPTY)
		{
			if (var.varType != VT_ULONG) throw IllegalArgumentException("Property PageSize must be VT_ULONG");
			m_pageSize = var.val.ulVal;
			m_nextPage = 0;
		}
		else
		{
			throw IllegalArgumentException("A new storage manager is created and property PageSize was not specified.");
		}
	}
	else
	{
		ssize_t bytesread = read(m_indexFile, &m_pageSize, sizeof(unsigned long));
		if (bytesread != sizeof(unsigned long)) throw IllegalStateException("Failed reading pageSize.");

		bytesread = read(m_indexFile, &m_nextPage, sizeof(long));
		if (bytesread != sizeof(long)) throw IllegalStateException("Failed reading nextPage.");
	}
	...
	if (bOverwrite == false)
	{
		...
		if (bytesread != sizeof(unsigned long)) throw IllegalStateException("Corrupted storage manager index file.");

		for (unsigned long cCount = 0; cCount < count; cCount++)
		{
			bytesread = read(m_indexFile, &page, sizeof(long));
			if (bytesread != sizeof(long)) throw IllegalStateException("Corrupted storage manager index file.");
...
void DiskStorageManager::flush()
{
	...
	if (seek < 0) throw IllegalStateException("Corrupted storage manager index file.");

	byteswritten = write(m_indexFile, &m_pageSize, sizeof(unsigned long));
	if (byteswritten != sizeof(unsigned long)) throw IllegalStateException("Corrupted storage manager index file.");
	byteswritten = write(m_indexFile, &m_nextPage, sizeof(long));
	if (byteswritten != sizeof(long)) throw IllegalStateException("Corrupted storage manager index file.");
...
void DiskStorageManager::loadByteArray(const long id, unsigned long& len, byte** data)
{
	...
	if (it == m_pageIndex.end()) throw InvalidPageException(id);
	...
	*data = new byte[len];
	...
	do
	{
		...
		if (seek < 0) throw IllegalStateException("Corrupted data file.");

		ssize_t bytesread = read(m_dataFile, m_buffer, m_pageSize);
		if (bytesread <= 0) throw IllegalStateException("Corrupted data file.");
...
void DiskStorageManager::storeByteArray(long& id, const unsigned long len, const byte* const data)
{
	if (id == NewPage)
	{
		...
		while (cRem > 0)
		{
			...
			if (byteswritten <= 0) throw IllegalStateException("Corrupted data file.");
	...
	else
	{
		...
		// check if it exists.
		if (it == m_pageIndex.end()) throw IndexOutOfBoundsException(id);
