...
//----------------------------------------------------------------------------
StandardMatrix4::StandardMatrix4 (const float aafEntry[4][4])
{
    memcpy(rowcol,aafEntry,16*sizeof(float));
}
//----------------------------------------------------------------------------
StandardMatrix4::StandardMatrix4 (const StandardMatrix4& rkMatrix)
{
    memcpy(rowcol,rkMatrix.rowcol,16*sizeof(float));
}
...
StandardMatrix4::StandardMatrix4(float etr1, float etr2, float etr3, float etr4,
...
//----------------------------------------------------------------------------
StandardMatrix4::StandardMatrix4 (const float entry[16])
{
    rowcol[0][0] = entry[0];
    rowcol[0][1] = entry[1];
    rowcol[0][2] = entry[2];
...
//----------------------------------------------------------------------------
float* StandardMatrix4::operator[] (int iRow) const
{
    return (float*)&rowcol[iRow][0];
}
//----------------------------------------------------------------------------
StandardMatrix4::operator float* ()
{
    return &rowcol[0][0];
}
//----------------------------------------------------------------------------
StandardVector4 StandardMatrix4::GetColumn (int iCol) const
{
    assert( 0 <= iCol && iCol < 4 );
    return StandardVector4(rowcol[0][iCol],rowcol[1][iCol],
        rowcol[2][iCol], rowcol[4][iCol]);
}
//----------------------------------------------------------------------------
  ...
  for (int iRow = 0; iRow < 4; iRow++)
      ...
      for (int iCol = 0; iCol < 4; iCol++)
	{
		 float f = 0;
            ...
            f += rowcol[iRow][0]*rkMatrix.rowcol[0][iCol];
...
//----------------------------------------------------------------------------
StandardMatrix4 StandardMatrix4::operator* (float fScalar) const
{
    StandardMatrix4 kProd;
    for (int iRow = 0; iRow < 4; iRow++)
    {
...
//----------------------------------------------------------------------------
StandardMatrix4 operator* (float fScalar, const StandardMatrix4& rkMatrix)
{
    StandardMatrix4 kProd;
    for (int iRow = 0; iRow < 4; iRow++)
    {
...
//----------------------------------------------------------------------------
/*
StandardMatrix4 StandardMatrix4::Inverse (float fTolerance) const
{
...
*/
//----------------------------------------------------------------------------
float StandardMatrix4::Determinant () const
{
  // to do
...
StandardMatrix4 TranslationMatrix4( float x, float y, float z )
{
...
StandardMatrix4 RotationXMatrix4( float x )
{
  StandardMatrix4 mat = StandardMatrix4::IDENTITY;
...
StandardMatrix4 RotationYMatrix4( float y )
{
  StandardMatrix4 mat = StandardMatrix4::IDENTITY;
...
StandardMatrix4 RotationZMatrix4( float z )
{
  StandardMatrix4 mat = StandardMatrix4::IDENTITY;
...
StandardMatrix4 RotationAxisMatrix4(float angle, float x, float y, float z)
{
...
StandardMatrix4 ScaleMatrix4( float xscale, float yscale, float zscale )
{
...
StandardMatrix4 RotationQuatMatrix4( float X, float Y, float Z, float W )
{
	StandardMatrix4 mat4 = StandardMatrix4::IDENTITY;
...
StandardMatrix4 OrthoMatrix4(float width, float height, 
                        float nearview, float farview) 
{
  StandardMatrix4 ret=StandardMatrix4::ZERO;
  float depth = farview-nearview;
...
/*!
 * \fn StandardMatrix4 PerspectiveMatrix4(float fov, float aspect, float nearz, float farz)
...
StandardMatrix4 PerspectiveMatrix4(float fov, float aspect, float nearz, float farz)
{
	float ymax = nearz*(float)tan(fov*PI/360);
	float ymin = -ymax;
...
StandardMatrix4 FrustumMatrix4(float left, float right, 
...
StandardMatrix4 PerspectiveFOVMatrix4(float angle, float aspect, 
    float nearview, float farview) 
{

  StandardMatrix4 ret=StandardMatrix4::ZERO;
  float depth = farview-nearview;
  angle = (float)(PI * (0.5f * angle)/360.0f);
  float cot = (float)cos(angle)/(float)sin(angle);
