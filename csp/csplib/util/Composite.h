...
public:
	 ...
	 *
	 *  This method is overloaded in the actual visitor classes to apply
	 *  distinct operations to each type of node.  Apply methods should
	 *  generally conclude with a call to traverse(node) to continue
	 *  progagation of the visitor through the graph.
	 */
...
protected:
	 ...
	 *
	 *  TRAVERSE_CHILDREN will visit the subgraph of the node (depth first),
	 *  while TRAVERSE_PARENTS will visit all parents until the root of the
	 *  graph is reached.
	 */
	 ...
	 *
	 *  The initial traversal mode can be set as a contructor parameter.  This
	 *  method allows the traversal mode to be changed on the fly, for example
	 *  to abort traversal of a graph once a condition is met (such as finding
	 *  a matching node during a search).
	 */
 ...
 *
 *  Composite should be used as a base class for nodes in a Composite/Visitor
 *  design pattern.  First declare a context specific visitor class (see
 *  Visitor), then create use Composite<> as a base class for the primary node
 *  type.  Starting from a context specific visitor (see Visitor for details)
 *  named MyNodeVisitor, one would declare MyNode as:
 *
 *     class MyNode: public Composite<MyNodeVisitor> {
 *     public:
 *       CSP_VISITABLE(MyNodeVisitor);
 *       // the rest of MyNode's declaration...
 *     };
 *
 *  MyNode and each of its subclasses must define an accept() method, which is
 *  just a couple lines of boilerplate code handled by the CSP_VISITABLE macro
 *  (feel free to write it out by hand if you despise macros).  Defining
 *  accept() in each subclass allows the overloaded apply() method in
 *  MyNodeVisitor to be correctly selected based on the type of each node.
 *
 *  Notes:
 *
 *  Although Composite supports multiple parents, this capability can easily be
 *  disabled within a given context by overriding the addChild() method of the
 *  primary node class and testing that getNumOfParents() equals zero for each
 *  child to be added.
 *
 *  The current implementation does not support non- composite leaf nodes.  Any
 *  node can contain other nodes, although again this feature can be disabled
 *  for specific node classes simply by overriding addChild().
 *
...
public:
	...
	/** Accept a visitor.
	 *
	 *  This method must be implemented in each subclasses to ensure proper
	 *  dispatch of the visitor's apply methods.  See the Composite class
	 ...
	 *
	 *  Extend this method to inforce constraints on the graph structure, such
	 *  as preventing nodes from having multiple parents.
	 *
	 ...
	 *
	 *  The original node will be destroyed if it is not already referenced
	 *  elsewhere.
	 *
 ...
 *
 *  Subclass Visitor to define a custom visitor class for a specific context.
 *  Although somewhat counter intuitive, the template parameter must be the
 *  sub- class being defined.  For example:
 *
 *  @code
 *     class MyNode;
 *     class MySpecialNode;
 ...
 *  @endcode
 *
 *  Note first that the base class apply() method must be brought into scope
 *  with a using directive.  There is also a macro CSP_VISITOR() which does
 *  this if you prefer using macros for boilerplate.
 *
 *  The rest of the class should define apply() stubs for each of the node
 *  classes in the current context.  Each of these node classes must be forward
 *  declared, and the apply stubs should chain from subclass to baseclass,
 *  ending finally with a call to apply((Node&)node), where Node is a type
 *  defined by Visitor that will be a base class of MyNode as long as MyNode
 ...
 *
 *  The visitor traverses the graph until the match() method returns true.
 *  Implement this method in a derived class to specify the search condition.
 *  Traversal stops as soon as a match is found. After traversal the getNode()
 *  method can be used to retrieve the matching node (if any).
 */
...
/** A visitor class for searching a node graph.
 *
 *  This visitor is similar to FindVisitor, but retrieves all nodes that match
 *  the condition.
 */
template <class N, class V>
