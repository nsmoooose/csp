...
SpatialIndex::IStorageManager* SpatialIndex::StorageManager::returnMemoryStorageManager(PropertySet& ps)
...
MemoryStorageManager::MemoryStorageManager(PropertySet& ps)
{
}
...
void MemoryStorageManager::loadByteArray(const long id, unsigned long& len, byte** data)
{
	...
	{
		e = m_buffer.at(id);
		if (e == 0) throw InvalidPageException(id);
	}
...
void MemoryStorageManager::storeByteArray(long& id, const unsigned long len, const byte* const data)
{
	...
	else
	{
		...
		try
		{
			...
			if (e_old == 0) throw InvalidPageException(id);
		}
		catch (std::out_of_range)
		{
			throw InvalidPageException(id);
