...
class ImagePropertiesPane(FilePropertiesPane):
    ...
    def __init__(self, parent, document, *args, **kwargs):
        ...
        if image.IsOk():
            ...
            self.tree.AppendItem(imageSize_item, "Width", image=self.TreeImages()['width'], wnd=width_text)

            height_text = wx.TextCtrl(self.tree, value=height_value, size=(50, -1), style=wx.TE_READONLY)
            self.tree.AppendItem(imageSize_item, "Height", image=self.TreeImages()['height'], wnd=height_text)
