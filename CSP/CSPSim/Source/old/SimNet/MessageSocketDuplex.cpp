...
MessageSocketDuplex::MessageSocketDuplex(Port port) {
	...
	CSP_LOG(NETWORK, DEBUG, "MessageSocketDuplex.MessageSocketDuplex(port) - port: " << port);
	m_UDPReceiverSocket = new ost::UDPSocket();
...
MessageSocketDuplex::MessageSocketDuplex(ost::InetAddress & addr, Port port) {
	CSP_LOG(NETWORK, DEBUG, "MessageSocketDuplex.MessageSocketDuplex(addr,port) - addr: " << addr << ", port: " << port);
	m_UDPReceiverSocket = new ost::UDPSocket();
	m_receiverAddr = &addr;
	m_receiverPort = port;
...
int MessageSocketDuplex::sendto(NetworkMessage * message, ost::InetHostAddress * remoteAddress, Port * remotePort) {
	CSP_LOG(NETWORK, DEBUG, "MessageSocketDuplex::sentto(message,addr,port)");
	message->dumpMessageHeader();

	CSP_LOG(NETWORK, DEBUG, "MessageSocketDuplex::sentto(message,addr,port) - Setting Remote Peer");
	m_UDPSenderSocket->setPeer(*remoteAddress, *remotePort);

	CSP_LOG(NETWORK, DEBUG, "MessageSocketDuplex::sentto(message,addr,port) - Sending Network Packet");
#ifdef _MSC_VER
	...
	CSP_LOG(NETWORK, DEBUG, "MessageSocketDuplex::sendto(message,addr,port) - exiting");
}

int MessageSocketDuplex::recvfrom(NetworkMessage ** message) {
	CSP_LOG(NETWORK, DEBUG, "MessageSocketDuplex()::recvfrom() - Receving Network Packet");
...
int MessageSocketDuplex::sendto(NetworkMessage * message, NetworkNode * node) {
	CSP_LOG(NETWORK, DEBUG, "MessageSocketDuplex::sendto(message,node) - Sending Network Packet");
