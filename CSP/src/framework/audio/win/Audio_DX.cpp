...
// STANDARD FRAMEWORK
// -------------------------------------------
void  Audio_DX::SetSetup(void *setup)
{
  globals = (_AUDIO_INIT_STRUCT *)setup;
}

short Audio_DX::GetDependencyCount()
{
  return 1;
}

void  Audio_DX::GetDependencyDescription(short Index, _DEPENDENCY *Dependency)
{
...
short Audio_DX::SetDependencyData(short ID, void* Data)
{
  switch(ID)
  {
...
void  Audio_DX::GetSetup(void*setup)
{
  setup=globals;
}

short Audio_DX::CheckStatus()
{
  if(globals == 0)
  {
...
int Audio_DX::Init()
{
  ...
  lpds->GetCaps(&dscaps);
	//if (platform)
  dsrval = lpds->SetCooperativeLevel((HWND)platform->GetWindowHandle(), DSSCL_EXCLUSIVE );
  if (dsrval!=DS_OK) return(-1);
 ...
 */
int Audio_DX::Load_Sound(char *_filename)
{
  int        retcode, t;
  AudioData *data;
...
/*
*/
int Audio_DX::Cache_Sound(int _handle, char _static)
{
  int t;
...
/*
  Play the sound .. yeah
 */
int Audio_DX::Play_Sound(int _handle, int _volume, int _looped)
{
  LPDIRECTSOUNDBUFFER  sbuffer;
  AudioData           *dat;
...
/*
 */
int Audio_DX::Stop_Sound(int _handle)
{
...
int Audio_DX::Continue_Sound(int _handle, int _volume, int _looped)
{
  LPDIRECTSOUNDBUFFER  sbuffer;
  AudioData           *dat;
...
*/
int Audio_DX::UnCache_Sound(int _handle)
{
  DWORD status;
...
*/
int Audio_DX::Discard_Sound(int _handle)
{
  if (_handle<0 || _handle>m_listbuffer.size())
    return _SOUND_OUTOFRANGE;
 ...
 *
 */
LPDIRECTSOUNDBUFFER Audio_DX::CreateSB(AudioData *_data, int _static)
{
  DSBUFFERDESC	       dsbd2;
  PCMWAVEFORMAT        pcmwf;
...
/*
*/
int Audio_DX::ReleaseSB(int _handle) {
