#ifndef _WinNetwork__CPP_
#define _WinNetwork__CPP_

#include "WinNetwork.h"

/////////////////////////////////////////////////////////////////////////////////////////////////
WinNetwork::WinNetwork(SOCKET _initializedSocket)
{
...
short WinNetwork::Create(unsigned short _socketType,long &_ip, short &_port)
{
	...
	{
		WinNetwork::CheckWSAError(stdout,"WinNetwork::Create(...)");
...
/////////////////////////////////////////////////////////////////////////////////////////////////
short WinNetwork::SetupEvent(HWND hDlg) {
	if(WSAAsyncSelect(this->m_sock, hDlg, WM_SOCKETREAD, FD_READ) != SOCKET_ERROR) {
		return NETWORK_RET_OK;
	}
	else { return NETWORK_RET_ERROR; }
}

/////////////////////////////////////////////////////////////////////////////////////////////////
short WinNetwork::Close(void)
{
	if(this->m_sock != INVALID_SOCKET)
	{
		if(closesocket(this->m_sock) == SOCKET_ERROR)
		{
			WinNetwork::CheckWSAError(stdout,"WinNetwork::Close(...)");
			return NETWORK_RET_ERROR;
		}
...
/////////////////////////////////////////////////////////////////////////////////////////////////
short WinNetwork::Listen(short _maxConnectionNr)
{
	if(listen(this->m_sock,_maxConnectionNr) == SOCKET_ERROR)
	{
		WinNetwork::CheckWSAError(stdout,"WinNetwork::Listen(...)");
		return NETWORK_RET_ERROR;
	}
...
/////////////////////////////////////////////////////////////////////////////////////////////////
short WinNetwork::Accept(StandardNetwork **_s, const long _mSecs)
{
	...
	case SOCKET_ERROR:
		WinNetwork::CheckWSAError(stdout,"WinNetwork::Accept(...)");
		return NETWORK_RET_ERROR;
	default:
...
/////////////////////////////////////////////////////////////////////////////////////////////////
short WinNetwork::Connect(long _ip,short _port)
{
	...
	if(connect(this->m_sock,(struct sockaddr *)&m_connectAddress,sizeof(m_connectAddress)) == SOCKET_ERROR)
	{
		...
		WinNetwork::CheckWSAError(stdout,"WinNetwork::Connect(...)");
		return NETWORK_RET_ERROR;
	}
...
/////////////////////////////////////////////////////////////////////////////////////////////////
short WinNetwork::Send(const char *_packet,const short _packetSize)
{
	if(send(this->m_sock,_packet,_packetSize,0) == SOCKET_ERROR)
	{
		WinNetwork::CheckWSAError(stdout,"WinNetwork::Send(...)");
		return NETWORK_RET_ERROR;
	}
	return NETWORK_RET_OK;
}
...
/////////////////////////////////////////////////////////////////////////////////////////////////
short WinNetwork::SendTo(char *_packet, const short _packetSize, long _ip, short _port)
{
	struct sockaddr_in addr;
	...
	{
		WinNetwork::CheckWSAError(stdout,"WinNetwork::SendTo(...)");
		return NETWORK_RET_ERROR;
	}
...
/////////////////////////////////////////////////////////////////////////////////////////////////
short WinNetwork::Receive(char *_packet,short *_packetSize, const long _mSecs)
{
	fd_set fdSet;
	struct timeval tVal;
	short ret = -1;
	...
	case 0:
		return NETWORK_RET_TIMEOUT;
	case SOCKET_ERROR:
		WinNetwork::CheckWSAError(stdout,"WinNetwork::Receive(...)");
		return NETWORK_RET_ERROR;
	default:
		char* test = _packet;
		short* test2= _packetSize;
		int test3 = this->m_sock;
		//was:
		//ret = recv(this->m_sock, _packet, *_packetSize, 0);
		ret = recv(m_sock,_packet,(int)_packetSize, 0);
		if(ret == SOCKET_ERROR)
		{
			WinNetwork::CheckWSAError(stdout,"WinNetwork::Receive(...)");
			return NETWORK_RET_ERROR;
		...
		else
		{
			//was:
			//*_packetSize = ret;
			_packetSize = (short*)ret;
			if(ret == 0) return NETWORK_RET_ERROR;
			else		 return NETWORK_RET_OK;
		}
		///notes by marve: the pointers used in the 'original' were causing an access violation.
	}
}

/////////////////////////////////////////////////////////////////////////////////////////////////
short WinNetwork::ReceiveFrom(char *_packet,short *_packetSize, long _ip, short _port,const long _mSecs)
{
	fd_set fdSet;
	struct timeval tVal;
	short ret = -1;
	...
	case 0:
		return NETWORK_RET_TIMEOUT;
	case SOCKET_ERROR:
		WinNetwork::CheckWSAError(stdout,"WinNetwork::ReceiveFrom(...)");
		return NETWORK_RET_ERROR;
	default:
		ret = recvfrom(this->m_sock,_packet,(int)_packetSize,0,(struct sockaddr *)&addr,&len);
		if(ret == SOCKET_ERROR)
		{
			WinNetwork::CheckWSAError(stdout,"WinNetwork::ReceiveFrom(...)");
			return NETWORK_RET_ERROR;
...
/////////////////////////////////////////////////////////////////////////////////////////////////
short WinNetwork::GetPeerAddress(long &_ip,short &_port)
{
	struct sockaddr_in addr;
	int len = sizeof(addr);
	...
	if(getpeername(this->m_sock,(struct sockaddr *)&addr,&len) == SOCKET_ERROR)
	{
		_ip = 0;
		_port = 0;
		WinNetwork::CheckWSAError(stdout,"WinNetwork::GetPeerAddress(...)");
		return NETWORK_RET_ERROR;
	}
...
/////////////////////////////////////////////////////////////////////////////////////////////////
short WinNetwork::GetSockAddress(long &_ip,short &_port)
{
	...
	if(getsockname(this->m_sock,(struct sockaddr *)&m_localAddress,&len) == SOCKET_ERROR)
	{
		...
		WinNetwork::CheckWSAError(stdout,"WinNetwork::GetSockAddress(...)");
		return NETWORK_RET_ERROR;
	}
...
long WinNetwork::ResolveHostname(char *name)
{
	struct hostent *h = gethostbyname(name);
	if(!h) {
		WinNetwork::CheckWSAError(stdout, "WinNetwork::GetHostByName(...)");
		return -1;
	}
	long ip = 0;
...
/////////////////////////////////////////////////////////////////////////////////////////////////
/*struct hostent *WinNetwork::GetHostByName(const char *_name)
{
...
/////////////////////////////////////////////////////////////////////////////////////////////////
struct hostent *WinNetwork::GetHostByAddress(struct sockaddr_in &_addr)
{
	struct hostent *h = gethostbyaddr((char *)&(_addr.sin_addr.S_un.S_un_b),4,0);

	if(!h)
	{
		WinNetwork::CheckWSAError(stdout,"WinNetwork::GetHostByAddress(...)");
		return NULL;
	}
	return h;
}
...
//////////////////////////////////////////////////////////////////////
short WinNetwork::CheckWSAError(FILE *_fp,char *_string)
{
...
void WinNetwork::PrintHostData(FILE *fp,struct hostent *h)
{
	fprintf(fp,"Host:\n");
...
int protNr=0;
	...
	while(doAgain)
	{
		...
		{
			err=WinNetwork::CheckWSAError(stdout);
			if(err==WSAENOBUFS)
			{
...
/*	struct hostent *hostEnt=gethostbyname("localhost");
	if(!hostEnt)
	{
		WinNetwork::CheckWSAError(stdout);
		return NETWORK_RET_ERROR;
	}else
