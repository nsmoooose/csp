...
class CustomLayoutModel(Node):
	...
	def makeGroup(self, node_map):
		...
		if self.__group is None:
			...
			if group is None:
				self.__group = cLayout.LayoutGroup()
				node_map.cacheGroup(self.xmlpath(), self.__group)
			else:
				self.__group = cLayout.LayoutGroup(group)
				cloned = 1
...
class FeatureObjectModel(Node):
	...
	def makeModel(self):
		if self.__model is None:
			...
			self.__model = cLayout.LayoutModel()
			self.__model.setName(str(self.xmlpath()))

			if ObjectInterface.ok:
				...
				self.__model.load(path, object_model.scale, offset[0], offset[1], offset[2], axis_0[0], axis_0[1], axis_0[2], axis_1[0], axis_1[1], axis_1[2])
...
class RandomBillboardModel(Node):
	def makeModel(self):
		if self.__model is None and ObjectInterface.ok:
