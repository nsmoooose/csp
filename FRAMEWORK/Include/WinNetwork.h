#ifndef _WinNetwork__H_
#define _WinNetwork__H_
 ...
 *	The server uses WinNetwork::Create to establish a socket connected to local address and defined port.
 *	Next it uses WinNetwork::Listen to inform the system that it await connect requests from clients. 
 *	Now this network is only for receiving connection requests and build up new connections.  
 *	With WinNetwork::Accept the server is checking for this requests It must do this periodicaly.
 *	If a client want to connect, the server receives that on WinNetwork::Accept an build up an new connection.
 *	This new connection will used to communicate with this special client.
 *	The client uses WinNetwork::Create to establish a socket connected to local address. The port should set to 0.
 *	If port is set to 0 the system is selecting an free port. The next step is to connect with WinNetwork::Connect to the server.
 *	After establishing these connection the client can communicate with defined server.<br>
 *	- Using UDP/IP:<br>That's more simple. At first use WinNetwork::Create to establish a socket connected to local address.
 *	Servers must define the port (clients should use port = 0). 
 *	After that client and server could use WinNetwork::Connect to define a default destination. So WinNetwork::Send and WinNetwork::Receive are usable.
 *	Without doing that, only WinNetwork::SendTo and WinNetwork::ReceiveFrom to receive from or send to a special destination.
 *	Without specifying the default destination WinNetwork::Receive will receive from all destinations (I think so :)). That's usefull on servers.
 *
 ...
 *	\todo Add on some methods time-depended execution. (WinNetwork::Send, WinNetwork::SendTo, WinNetwork::Connect).
 */
class WinNetwork:public StandardNetwork{
public:
    ...
    */
	WinNetwork(SOCKET _initializedSocket = INVALID_SOCKET);
    //! A destructor.
    /*!
      It destroys the socket. It calls WinNetwork::Close() for that.
    */
	~WinNetwork(void);
	//@}
	...
	*/
	...
	//!Sets up a Windows Dialog Event so we can receive packets when there's data to receive
	short SetupEvent(HWND hDlg);
	...
	*/
	...
	/*! This socket will used after execution only for initial handshaking between client and server. 
		So this method is ever used together with WinNetwork::Accept.
	...
	*/
	...
	/*! Before using this method WinNetwork_Listen must be executed. The method is _mSecs microseconds waiting for an incoming connection request.
	...
	*/
	...
	//! Build up a connection to foreign address (TCP/IP) or define a default destination for StandardNetwork::Send and WinNetwork::Receive (UDP/IP).
	/*! On UDP/IP: Any datagrams received from an address other than the destination address specified will be discarded. 
	If the ip-address is zero, the socket will be disconnected. Then no default destination is set and only WinNetwork::SendTo and WinNetwork::ReceiveFrom are usable.<br>
	...
	*/
	...
	//! Receives packets. If default destination is set (WinNetwork::Connect) it receives only packets from this address. 
	...
	*/
	...
	For UDP/IP, only the address of a peer specified in a previous WinNetwork::Connect call will be returned.
	Any address specified by a previous WinNetwork::SendTo call will not be returned by this method.
	...
	*/
	...
	//@}
	long ResolveHostname(char* name);
protected:
