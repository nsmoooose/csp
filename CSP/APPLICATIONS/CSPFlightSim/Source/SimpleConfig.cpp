...
public:
	...
	virtual void write(std::ostream &os) const {
		os << SECTION_START << m_section << SECTION_END << endl;
	}
...
void SimpleConfig::dump(ostream &os) const {
...
int SimpleConfig::getInt(const std::string &section, const std::string &key, int default_=0, bool set) {
...
double SimpleConfig::getFloat(const std::string &section, const std::string &key, double default_=0.0, bool set) {
	ConfigValue *v = _find(section, key);
	if (v) {
...
void SimpleConfig::open(const char *fn) {
	if (fn) {
		std::filebuf fb;
		if (m_modified) save();
		clear();
