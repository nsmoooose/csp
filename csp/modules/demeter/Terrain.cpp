...
TerrainLattice::TerrainLattice(const char* szBaseName,const char* szExtensionElev,const char* szExtensionTex,const char* szDetailTextureName,float vertexSpacing,float elevationScale,int maxNumTriangles,bool bUseBorders,int widthTerrains,int heightTerrains)
{
	...
	Terrain* pTerrain;
	sprintf(szElevName,"%sElev.%d-%d.%s",m_szBaseName,0,0,m_szExtensionElev);
...
//    else
//    {
//        sprintf(szTexName,"%sTex.%d-%d.%s",m_szBaseName,0,0,m_szExtensionTex);
...
void TerrainLattice::GenerateTextures(TextureSet* /*pTextureSet*/,TextureFactory* pTextureFactory,int tilesWidth,int tilesHeight)
{
	...
	for (int i = 0; i < m_WidthTerrains * m_HeightTerrains; i++)
	{
		...
		sprintf(szTexName,"%sDetailTex.%d-%d.surface",m_szBaseName,indexX,indexY);
...
void TerrainLattice::LoadTerrain(int index)
{
	...
	sprintf(szElevName,"%sElev.%d-%d.%s",m_szBaseName,indexX,indexY,m_szExtensionElev);
	Terrain* pTerrain;
	if (m_pTextureFactory == NULL)
	{
		sprintf(szTexName,"%sTEX.%d-%d.%s",m_szBaseName,indexX,indexY,m_szExtensionTex);
...
void TerrainLattice::Render()
{
	...
	for (int i = 0; i < 9; i++)
	{
		if (m_CurrentTerrainIndex[i] != -1)
		{
			...
			glMatrixMode(GL_MODELVIEW);
			glPushMatrix();
			glTranslatef( m_OffsetIndexX[i]*m_TerrainWidth, m_OffsetIndexY[i]*m_TerrainHeight, 0);
...
DetailTexture::DetailTexture(Texture* pTexture)
{
	...
	if (pTexture)
	{
		...
		delete[] pBuffer;
		//DetailTexture* pDetail = new DetailTexture;  XXX not used?
		SetMask(pMask);
		SetTexture(pTexture);
	}
}
