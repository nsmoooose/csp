...
void GlStateSnapshot::_logDiff(std::string const &label, GlStateSnapshot const &other) {
	...
	for (unsigned i = 0; i < m_StateLog.size(); ++i) {
		if (m_StateLog[i] != other.m_StateLog[i]) {
			if (first) {
				CSPLOG(Prio_WARNING, Cat_SCENE) << "GL_STATE_DIFF -- " << label;
				first = false;
			}
			CSPLOG(Prio_WARNING, Cat_SCENE) << "GL_STATE_DIFF < " << m_StateLog[i];
			CSPLOG(Prio_WARNING, Cat_SCENE) << "GL_STATE_DIFF > " << other.m_StateLog[i];
		}
	}
}
