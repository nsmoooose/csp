...
void EchoMessageHandler::process(NetworkMessage * message) {
	CSP_LOG(NETWORK, DEBUG, "EchoMessageHandler::process()\n");

	NetworkNode * originatorNode = message->getOriginatorNode();
	CSP_LOG(NETWORK, DEBUG, "EchoMessageHandler::process() - Sending back to host: " << originatorNode->getHostname() << ", port: " << originatorNode->getPort());
