...
//	float dbcenter[3], float *dbradius;
//	dbcenter[0] = xcen;
//	dbcenter[1] = ycen;
//	dbcenter[2] = 0;
...
//    getDatabaseCenterRadius( dbcenter, &dbradius );
	...
	_tree *trees = new _tree[icnt*jcnt];
	...
	for (i=0;i<icnt;i++)
		for (j=0;j<jcnt;j++)
		{
			...
			int count = i*icnt + j;
			trees[i].n = count;
			trees[i].x = xoff + i*spacing;
			trees[i].y = yoff + j*spacing;
			trees[i].z = pBattlefield->getElevation(trees[i].x, trees[i].y);
			trees[i].h = 10;
			trees[i].w = 7.5;

            Vec3 pos( trees[i].x, trees[i].y, trees[i].z );
            GeoSet *gset = makeTree( &trees[i], dstate );
