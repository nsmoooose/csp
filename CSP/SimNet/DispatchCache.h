...
#ifndef __SIMNET_DISPATCHCACHE_H__
#define __SIMNET_DISPATCHCACHE_H__
 ...
 */
class DispatchCache: public simdata::Referenced {

	typedef simdata::uint64 CacheKey;
	typedef simdata::MostRecentlyUsedCache<CacheKey, BaseCallback::Ref, simdata::nohash64> MRUCache;
	 ...
	 */
	static inline simdata::uint64 makeCacheKey(simdata::uint64 target_id, simdata::uint64 message_id) {
...
public:
	typedef simdata::Ref<DispatchCache> Ref;
	 ...
	 */
	bool findHandler(NetworkMessage::Ref const &msg, BaseCallback::Ref &handler) {
	...
	void insert(CLASS *instance, typename MessageCallback<CLASS, MSG>::Method handler, NetworkMessage::Ref const &msg) {
	 ...
	 *  @param msg The message being handled.
	 */
	void insertNoHandler(NetworkMessage::Ref const &msg) {
		const CacheKey key = makeCacheKey(msg->getRoutingData(), msg->getId());
...
#endif // __SIMNET_DISPATCHCACHE_H__
