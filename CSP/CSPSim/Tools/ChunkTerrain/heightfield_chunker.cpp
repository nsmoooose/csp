...
//	check_propagation(hf, hf.size >> 1, hf.size >> 1, hf.log_size - 1);//xxxxx
	...
	SDL_WriteLE16(out, 10);	// file format version.
...
// careful.
// XXX added vertex count to header immediately after maxy
const int	CHUNK_HEADER_BYTES = 4 + 4*4 + 1 + 2 + 2 + 2*2 + 2 + 4;
...
//code:
	...
	// Write out the current chunk.
	{
		...
		// we catch any overflows a few lines down
		SDL_WriteLE16(rw, (unsigned short) vertices.size());
