...
CSP_TESTFIXTURE(Vector3) {
	...
	CSP_TESTCASE(Axes) {
		CSP_EXPECT_FEQ(0, delta(Vector3(1, 0, 0), Vector3::XAXIS));
		CSP_EXPECT_FEQ(0, delta(Vector3(0, 1, 0), Vector3::YAXIS));
		CSP_EXPECT_FEQ(0, delta(Vector3(0, 0, 1), Vector3::ZAXIS));
	}

	CSP_TESTCASE(Arithmetic) {
		Vector3 v(1, 2, -3);
		CSP_EXPECT_FEQ(0, delta(Vector3(2.5, 5, -7.5), 2.5 * v));
		CSP_EXPECT_FEQ(0, delta(Vector3(2.5, 5, -7.5), v * 2.5));
		CSP_EXPECT_FEQ(0, delta(Vector3(2.5, 5, -7.5), v / 0.4));
		CSP_EXPECT_FEQ(0, delta(Vector3(-1, -2, 3), -v));
		CSP_EXPECT_FEQ(0, delta(Vector3(3, 6, -9), v + 2 * v));
		CSP_EXPECT_FEQ(0, (v - v).length());
		v += 1.5 * v;
		CSP_EXPECT_FEQ(0, delta(Vector3(2.5, 5, -7.5), v));
		v /= 2.5;
		CSP_EXPECT_FEQ(0, delta(Vector3(1, 2, -3), v));
		v *= 2.5;
		CSP_EXPECT_FEQ(0, delta(Vector3(2.5, 5, -7.5), v));
	...
	CSP_TESTCASE(DotProduct) {
		CSP_EXPECT_FEQ(0.1, Vector3(4, -1, -1) * Vector3(0.25, 0.5, 0.4));
		CSP_EXPECT_FEQ(0.1, dot(Vector3(4, -1, -1), Vector3(0.25, 0.5, 0.4)));
		CSP_EXPECT_FEQ(-0.5, Vector3(1, 2, 3) * Vector3(-0.5, -6, 4));
		CSP_EXPECT_FEQ(-0.5, dot(Vector3(1, 2, 3), Vector3(-0.5, -6, 4)));
	}

	CSP_TESTCASE(CrossProduct) {
		CSP_EXPECT_FEQ(0, delta(Vector3(0.1, -1.85, 2.25), Vector3(4, -1, -1) ^ Vector3(0.25, 0.5, 0.4)));
		CSP_EXPECT_FEQ(0, delta(Vector3(0.1, -1.85, 2.25), cross(Vector3(4, -1, -1), Vector3(0.25, 0.5, 0.4))));
		CSP_EXPECT_FEQ(0, delta(Vector3(26, -5.5, -5), Vector3(1, 2, 3) ^ Vector3(-0.5, -6, 4)));
		CSP_EXPECT_FEQ(0, delta(Vector3(26, -5.5, -5), cross(Vector3(1, 2, 3), Vector3(-0.5, -6, 4))));
	}
	...
	CSP_TESTCASE(Bound) {
		...
		CSP_EXPECT_FEQ(0, delta(Vector3(0, 0, 0), minv));
		CSP_EXPECT_FEQ(0, delta(Vector3(1, 2, 3), maxv));

		Vector3(2, 4, 6).bound(minv, maxv);
		CSP_EXPECT_FEQ(0, delta(Vector3(0, 0, 0), minv));
		CSP_EXPECT_FEQ(0, delta(Vector3(2, 4, 6), maxv));

		Vector3(-1.1, 3.1, 7.1).bound(minv, maxv);
		CSP_EXPECT_FEQ(0, delta(Vector3(-1.1, 0, 0), minv));
		CSP_EXPECT_FEQ(0, delta(Vector3(2, 4, 7.1), maxv));
		...
		CSP_EXPECT_FEQ(0, delta(Vector3(-2.1, -3.1, -4.1), minv));
		CSP_EXPECT_FEQ(0, delta(Vector3(2, 4, 7.1), maxv));
	}
