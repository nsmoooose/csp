...
///////////////////////////////////////////////////////////////////////////
//
//   Base code based upon Jeff Molofee's Revised OpenGL Basecode
...
#include "BaseCode.h"													// Header File For The NeHeGL Basecode
...
CCamera		 *m_pCamera;							
CDlgStartup	  m_cDlgStartup;
CDlgProcess	  m_cDlgProcess;
CDlgExtract   m_cDlgExtract;
CFractal	Fractal;
...
//			PostMessage (hWnd, WM_QUIT, 0, 0);
		...
		case WM_MOUSEMOVE:
			// set camera heading
			if ((wParam & MK_LBUTTON) && !(wParam & MK_RBUTTON)) 		// LOWORD contains MouseX, HIWORD MouseY
			{
				m_pCamera->m_fRotX -= LOWORD(lParam) - iLastMouseposX;
				m_pCamera->m_fRotY += HIWORD(lParam) - iLastMouseposY;
			}
			// set sun position
			if ((wParam & MK_RBUTTON) && !(wParam & MK_LBUTTON))		// LOWORD contains MouseX, HIWORD MouseY
			{
				m_pTerrainData->m_faLightPos[0] += (LOWORD(lParam) - iLastMouseposX) * 50;
				m_pTerrainData->m_faLightPos[1] -= (HIWORD(lParam) - iLastMouseposY) * 50;
			}
			if ((wParam & MK_RBUTTON) && (wParam & MK_LBUTTON))			// LOWORD contains MouseX, HIWORD MouseY
			{
				m_pTerrainData->m_faLightPos[2] += (LOWORD(lParam) - iLastMouseposX) * 50;
			}
			iLastMouseposX = LOWORD(lParam);							// always update mouse position to avoid
			iLastMouseposY = HIWORD(lParam);							// a "jumping" camera
		break;
	}
	...
	while (g_isProgramLooping)											// Loop Until WM_QUIT Is Received
	{
		...
		if (CreateWindowGL (&window) == TRUE)							// Was Window Creation Successful?
		{
			...
			{	// Initialize was a success
				...
				SetWindowPos(window.hWnd, HWND_TOP, 50, 50, 800, 600, 0);

				// Initialize all data structures
				retval = m_cDlgStartup.QueryParameters();
				if(!retval)
					TerminateApplication(&window);						// First chance to quit the app
				
				// VSync or not?
				if (cSwapInterval.InitSwapIntervalExt())
