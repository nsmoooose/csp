...
public:
    ...
    int receive(void* buffer, std::size_t len, const std::chrono::duration<Rep, Period>& timeout) {
        ...
        m_socket.async_receive(boost::asio::buffer(buffer, len),
...
public:
	...
	inline bool isOutputReady() {
		boost::system::error_code ec;
		/* TODO This a blocking call. Should we keep that? The original wasn't
		   blocking. */
		m_socket.wait(boost::asio::socket_base::wait_write, ec);
		return !ec;
	}
