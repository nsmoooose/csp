...
void AutoTrace::__sigsegv(int /*sig_n*/) {
	...
	LogStream::LogEntry(*logstream, LogStream::cError, Cat_NONE) << "FATAL ERROR: segmentation fault";
...
void AutoTrace::__sigabort(int /*sig_n*/) {
	if (_abort) {
		...
		_trace.acquire(/*skip=*/3);
		LogStream *logstream = log();
		LogStream::LogEntry(*logstream, LogStream::cFatal, Cat_GENERIC) << "ABORT";
	}
}
...
void AutoTrace::__sigterm(int /*sig_n*/) {
	...
	LogStream *logstream = log();
	_abort = false;
	LogStream::LogEntry(*logstream, LogStream::cFatal, Cat_GENERIC) << "TERMINATED";
}
